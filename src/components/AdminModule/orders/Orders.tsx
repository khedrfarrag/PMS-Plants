...
function Orders() {
  ...
  const getAllProducts = async () => {
    try {
      ...
      const response = await axios.get<any>(
        ProductsPoint.GetAllProducts + `?pageNumber=1&pageSize=999999`,
        {
          headers: {
            Authorization: `Bearer ${
              localStorage.getItem("token") || sessionStorage.getItem("token")
  ...
  const updateOrder = async () => {
    ...
    if (!editOrder.OrderItems || editOrder.OrderItems.length === 0)
      missing.push("منتج واحد على الأقل");
    if (missing.length) {
    ...
    try {
      ...
      await axios.put(`${ordersPoint.Put(selectedOrder.Id)}`, orderData, {
        headers: {
          Authorization: `Bearer ${
            localStorage.getItem("token") || sessionStorage.getItem("token")
          }`,
  ...
  const createOrder = async () => {
    ...
    if (!addOrder.OrderItems || addOrder.OrderItems.length === 0)
      missing.push("منتج واحد على الأقل");
    if (missing.length) {
      toast.error(`من فضلك أكمل الحقول: ${missing.join("، ")}`);
      return;
    ...
    try {
      ...
      await axios.post(ordersPoint.Post, orderData, {
        headers: {
          Authorization: `Bearer ${
            localStorage.getItem("token") || sessionStorage.getItem("token")
          }`,
  ...
  return (
    ...
    >
      <header className={Style.orderHeader}>إدارة الطلبات</header>
      {loading ? (
        <div
          style={{
            display: "flex",
        ...
        >
          ...
          >
            ...
            <div style={{ overflowX: "auto" }}>
              <table
                className={Style.orderTable + " align-middle text-center"}
                style={{ width: "100%" }}
              >
                <thead>
                  <tr>
            ...
            >
              <Modal.Header closeButton>
                <Modal.Title className="d-flex justify-content-between align-items-center w-100">
                  <span style={{ fontSize: "1.1rem", fontWeight: "normal" }}>
                    تفاصيل الطلب
                  </span>
                  {selectedOrder && (
                    <span
                      className={`badge ${
                        getStatusInfo(selectedOrder.PaymentStatus).className
                      }`}
                      style={{
                        padding: "6px 12px",
                        borderRadius: "8px",
                                  ...
                                  <td>
                                    <input
                                      ...
                                      onChange={(e) => {
                                        ...
                                        setEditOrder({
                                          ...editOrder,
                                          OrderItems: editOrder.OrderItems.map(
                                            (it: any, i: number) =>
                                              i === idx
                                                ? { ...it, Quantity: val }
                                                : it
                        ...
                        <div className="d-flex align-items-center gap-2 mb-2">
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              if (
                                editOrder.OrderItems.some(
                                  (it: any) => it.ProductId === newProductId
                                )
                              )
                                return;
                              setEditOrder({
                                  ...
                                  <td>
                                    <input
                                      ...
                                      onChange={(e) => {
                                        ...
                                        setAddOrder({
                                          ...addOrder,
                                          OrderItems: addOrder.OrderItems.map(
                                            (it: any, i: number) =>
                                              i === idx
                                                ? { ...it, Quantity: val }
                                                : it
                ...
                {modalMode === "edit" && (
                  <>
                    ...
                    <button
                      ...
                      onClick={() =>
                        selectedOrder && deleteOrder(selectedOrder.Id)
                      }
                    >
                      حذف الطلب
                    </button>
