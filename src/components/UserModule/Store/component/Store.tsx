...
import { faCheckCircle, faExclamationTriangle, faStar } from "@fortawesome/free-solid-svg-icons";
import {
...
if (categoryIdFromURL) {
  setFilters(prev => ({ 
    ...
    SubCategoryId: subCategoryIdFromURL ? parseInt(subCategoryIdFromURL, 10) : undefined
  }));
      setSelectedCategoryId(parseInt(categoryIdFromURL, 10));
      setSelectedSubCategoryId(subCategoryIdFromURL ? parseInt(subCategoryIdFromURL, 10) : null);
    }
  ...
  // تحسين: استخدام useCallback للـ pagination
                  ...
                  <div className={Style.hedcard}>
                    ...
                    >
                      <FontAwesomeIcon
                        icon={faHeart}
                        style={{
                          color: favorites.includes(card?.Id) ? "red" : "white",
                    ...
                    {getStockStatus(card.StockQuantity).status === "lastPiece" && (
                    ...
                    {getStockStatus(card.StockQuantity).status === "lowStock" && (
                    ...
                    {getStockStatus(card.StockQuantity).status === "outOfStock" && (
                    ...
                    {getStockStatus(card.StockQuantity).status === "inStock" && (
