...
export default function Popular() {
  ...
  const getDiscount = async (PageSize: number, CurrentPage: number) => {
    try {
      ...
      }>(ProductsPoint.GetPopular(CurrentPage, PageSize), {
        headers: {
  ...
  const incrementHandler = useCallback(
    (Id: number): void => {
      const product = getpopular.find((p) => p.Id === Id);
      if (!product) return;
    ...
    [counts, getpopular, canAddToCart, getStockMessage]
  );
  ...
  return (
    <>
      <div className={`${Styles.contanerseller} `}>
        <div className={`${Styles.captionseller} mt-5 `}>
          <h1 className="text-center text-warning">منتجات الشهر</h1>
