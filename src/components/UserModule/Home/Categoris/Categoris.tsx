...
export default function Categoris() {
  ...
  const getAllCategories = async ({ pageNumber, pageSize }: { pageNumber: number; pageSize: number }) => {
    try {
      setLoading(true);
  ...
  // دالة للتنقل للفئة
  const goToCategory = (category: any, isMainCategory: boolean, parentCategory?: any) => {
    if (isMainCategory) {
      // فئة رئيسية
      navigate(`/store?categoryId=${category.Id}`);
    } else {
      // فئة فرعية - نحتاج معرف الفئة الرئيسية أيضاً
      navigate(`/store?categoryId=${parentCategory.Id}&subCategoryId=${category.Id}`);
    }
    window.scrollTo(0, 0);
  };
  ...
  return (
    <>
      <div className={Style.captioncateg}>
        ...
        <p className="text-center">اختر الفئة المطلوبة أو أحد الفئات الفرعية المرتبطة بها</p>
      </div>

      <div 
        className="d-flex flex-wrap align-items-stretch gap-3" 
        ...
        <div className="flex-grow-1" style={{ minWidth: 260 }}>
             ...
             <AnimatePresence mode="wait">
               {subCats.slice(0, 4).map((sc, index) => (
                 <motion.button
                   ...
                   whileHover={{ 
                     scale: 1.05, 
                     background: "linear-gradient(180deg, rgba(16, 181, 65, 0.06), rgba(69, 197, 107, 0.02))",
                     boxShadow: "0 4px 12px rgba(0, 238, 72, 0.15)",
                     transition: { duration: 0.2 }
                   }}
                 >
                   <div className="d-flex align-items-center justify-content-center mb-2" style={{ height: 60}}>
                  {sc?.Name && (<img
         ...
         <AnimatePresence mode="wait">
           <motion.div
             ...
             className={`bg-white shadow rounded-4 d-flex flex-column justify-content-between`}
             style={{
               minWidth: 280,
               width: 360,
               flex: "0 0 360px",
           ...
           >
             <div className="d-flex align-items-center gap-3">
               ...
               >
                 <h5 className="m-0" style={{ fontWeight: 800, color: "#0a0a0a" }}>
                   {activeCategory.Name}
                 </h5>
                 <small style={{ color: "#607d8b" }}>
                   {subCats.length ? `${subCats.length} فئات فرعية` : "بدون فئات فرعية"}
                 </small>
               </motion.div>
             </div>
